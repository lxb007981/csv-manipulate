...
function parseSetInput(input: string): { row: string, col: string, target: string } {
	const [row, col, target] = input.split(',');
	return {
		row: row.trim(),
		col: col.trim(),
		target: target.trim()
	...
	const setCellValueDisposable = vscode.commands.registerCommand('extension.setCell', async function () {
		...
		if (editor) {
			...
			*/
			...
			const colNumber = document.lineAt(0).text.split(',').indexOf(col);
			if (colNumber === -1) {
			...
			/* find the cell start */
			targetRow.text.split(',').find((cell, index) => {
				if (index === colNumber) {
					cellVal = cell;
					return true;
				}
		...
		if (editor) {
			...
			*/
			...
			document.getText().split('\n').find((line, _) => {
				const firstComma = line.indexOf(',');
				if (firstComma !== -1 && line.substring(0, firstComma) === row) {
					vscode.window.showInformationMessage(`value: ${line.split(',')[colNumber]}, row: ${row}, col: ${col}`);
					return true;
				}
			});
